...
function erp_acct_insert_purchase( $data ) {
    ...
    try {
        ...
        foreach ( $items as $item ) {
            // update stock
            $stockQuantity = $wpdb->get_var("SELECT quantity FROM wp_fgpt_productstock WHERE prod_id = ".$item['product_id']);
            $stockQuantity = $stockQuantity + $item['qty'];
            $resultPropStock = $wpdb->update('wp_fgpt_productstock', array('quantity'=> $stockQuantity), array('prod_id'=>$item['product_id']));
